...
type Tab = "overview" | "microcap" | "indices" | "cashflow" | "animal" | "liquidity";
...
function LiquidityPage() {
  return <><ThemeHeading kicker="Liquidity Profiles · 跨市场研究" title="流动性不是一个数字，而是可交易规模、市场结构和数据覆盖的组合。" text="market-research 已吸收 liquidity profiles 的面板契约、lagged ADV/MedADV、机械容量和质量诊断。原始行情不发布，公开页面只承载脱敏汇总。" asof="本地数据待刷新"/><div className="market-grid">{[["A股", "面板适配器已完成", "本地报告待公开快照"], ["港股", "面板适配器已完成", "需要本地数据刷新"], ["美股", "面板适配器已完成", "需要本地数据刷新"], ["日股", "JPX/J-Quants适配器已完成", "需要本地数据刷新"]].map(([market, status, note]) => <article className="market-card" key={market}><span>{market}</span><strong className="pending">{status}</strong><small>{note}</small></article>)}</div><div className="research-grid"><article className="research-card"><span className="section-kicker">定义</span><h3>成交额不等于容量</h3><p>页面将名义成交、滞后 ADV、MedADV、参与率和冲击成本拆开，避免用成交额直接代替可执行规模。</p></article><article className="research-card"><span className="section-kicker">时间</span><h3>特征严格滞后一天</h3><p>流动性特征不使用当日成交额，防止把当天结果泄漏进交易前的可投资性判断。</p></article><article className="research-card"><span className="section-kicker">数据边界</span><h3>公开派生结果</h3><p>移动硬盘和本地市场数据不会进入 GitHub；刷新后只发布市场级汇总、覆盖期和质量状态。</p></article></div></>;
}
...
function App() {
  const validTabs: Tab[] = ["overview", "microcap", "indices", "cashflow", "animal", "liquidity"];
