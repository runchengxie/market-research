...
function CashflowPage() {
  ...
  const comparison = rows.filter((row) => row.window === windowKey && (basis === "all" || row.return_basis === basis));
...
function LiquidityPeriodPanel({ embedded }: { embedded: boolean }) {
  ...
  return <section className={`period-panel ${embedded ? "embedded" : ""}`}><div className="period-heading"><div><span className="section-kicker">跨市场小微盘流动性</span><h3>按数据日期比较小市值股票的流动性</h3><p>各市场按自身数据日期分桶，使用滞后 20 日平均成交额。由于数据更新不同步，页面单独列出每个市场的日期。</p></div><span className={`status-badge ${periodStatus}`}>{periodStatus === "verified" ? "口径已核验" : periodStatus === "mixed" ? "日期不一致" : "覆盖不完整"}</span></div><ControlBar><span className="control-label">时间区间</span>{options.map((value) => { const item = value === "latest" ? null : summary.periods?.find((candidate) => candidate.period === value); return <Choice key={value} active={period === value} onClick={() => setPeriod(value)}>{value === "latest" ? "最新快照" : value}{item?.status === "incomplete" ? " · 不完整" : ""}</Choice>; })}</ControlBar><div className="period-meta">{selected ? `${selected.common_start ?? "—"} 至 ${selected.common_end ?? "—"} · ${available.length}/${markets.length} 个市场可比` : `各市场最近可用快照 · ${latestDates || "日期待补充"}`}</div>{selected && selected.status === "incomplete" && <div className="callout compact"><span className="section-kicker">覆盖提醒</span><p>该区间的市场覆盖范围不完全一致，下面只展示有数据的市场，缺失市场不会填成零。</p></div>}<div className="period-table"><SimpleTable rows={available.flatMap((market) => market.buckets.map((bucket) => ({ market: `${market.market}（截至 ${selected ? market.coverage_end ?? "日期待补" : market.as_of ?? "日期待补"}）`, bucket: bucket.label, median_usd: `$${num(bucket.median_usd)}`, mean_usd: `$${num(bucket.mean_usd)}`, observations: num(bucket.observations) })))} columns={[["market", "市场"], ["bucket", "市值分位"], ["median_usd", "成交中位"], ["mean_usd", "成交均值"], ["observations", "观测数"]]} /></div></section>;
}
