...
type Tab = "microcap-recovery" | "cashflow-recovery" | "overview" | "microcap" | "style" | "cashflow" | "cross-market" | "indices" | "liquidity";
...
function BarraPage() {
  ...
  if (!summary || !quantiles || !factors || !yearly || !correlations) return <><ThemeHeading kicker="历史研究档案 · Barra 风格因子" title="A 股风格因子的长期历史表现" text="查看历史研究的方法、因子表现、逐年收益和相关性，最后补充市值十分组研究。" asof="历史快照加载中"/><div className="callout status-panel"><span className="section-kicker">研究状态</span><h3>历史研究快照正在加载</h3><p>{summaryError || quantilesError ? "网页数据不完整，请先生成并发布 Barra 历史派生文件。" : "正在加载历史因子总览、逐年收益和相关性数据。"}</p></div></>;
  const monotonicity = summary.size_monotonicity;
...
function App() {
  const validTabs: Tab[] = ["cashflow-recovery", "overview", "microcap", "style", "cashflow", "cross-market", "indices", "liquidity"];
  const hash = (window.location.hash === "#microcap-recovery" ? "microcap" : window.location.hash.slice(1)) as Tab;
